...
// instantiate task
task("instantiate", "Instantiate a Calculator computation").setAction(
  async ({}, hre) => {
    const { ethers } = hre;
    const cartesiCompute = await ethers.getContract("CartesiCompute");
    const contract = await ethers.getContract("Calculator");
...
// get-result task
task("get-result", "Retrieves a Calculator computation result given its index")
  .addOptionalParam("index", "The Calculator computation index", 0, types.int)
...
// destruct task
task("destruct", "Destructs a Calculator computation")
  .addOptionalParam("index", "The Calculator computation index", 0, types.int)
  .setAction(async ({ index }, hre) => {
    const { ethers } = hre;
    const contract = await ethers.getContract("Calculator");
